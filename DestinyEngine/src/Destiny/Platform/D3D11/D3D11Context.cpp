...
Destiny::D3D11Context::D3D11Context(HWND hWnd, uint16_t width, uint16_t height,
  ...
  DT_D3D11_THROW_FAILED(D3D11CreateDeviceAndSwapChain(
      ...
      D3D11_SDK_VERSION, &swapChainDesc, m_SwapChain.ReleaseAndGetAddressOf(),
      m_Device.ReleaseAndGetAddressOf(), nullptr,
...
void Destiny::D3D11Context::swap() {
  ...
  if (FAILED(hr = m_SwapChain->Present(m_VSync ? 1 : 0, 0))) {
    if (hr == DXGI_ERROR_DEVICE_REMOVED) {
      throw DT_D3D11_DEVICE_REMOVED_EXCEPTION(
...
void Destiny::D3D11Context::clear() {
  ...
  // Draw test triangle
  {
    ...
    Ref<VertexBuffer> vertexBuffer = Ref<VertexBuffer>(
        VertexBuffer::create(reinterpret_cast<float *>(&vertices),
                             std::size(vertices) * 5, this, layout));
    vertexBuffer->bind();

    // Create index buffer
    uint32_t indices[] = {0, 1, 2, 0, 3, 1};
    Ref<IndexBuffer> indexBuffer = Ref<IndexBuffer>(
        IndexBuffer::create(indices, std::size(indices), this));
    indexBuffer->bind();

    // Create vertex shader
    ComRef<ID3D11VertexShader> vertexShader;
    ComRef<ID3DBlob> blob;
    DT_D3D11_THROW_FAILED(
        D3DReadFileToBlob(L"assets/shaders/hlsl/test_shader_vs.cso",
                          blob.ReleaseAndGetAddressOf()));
    DT_D3D11_THROW_FAILED(m_Device->CreateVertexShader(
    ...
    const D3D11_INPUT_ELEMENT_DESC ied[] = {
        ...
        {"COLOR", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 2 * sizeof(float),
         D3D11_INPUT_PER_VERTEX_DATA, 0},
    };

    DT_D3D11_THROW_FAILED(m_Device->CreateInputLayout(
        ied, (UINT)std::size(ied), blob->GetBufferPointer(),
    ...
    DT_D3D11_THROW_FAILED(
        D3DReadFileToBlob(L"assets/shaders/hlsl/test_shader_fs.cso",
                          blob.ReleaseAndGetAddressOf()));
    DT_D3D11_THROW_FAILED(m_Device->CreatePixelShader(
        blob->GetBufferPointer(), blob->GetBufferSize(), nullptr,
...
void Destiny::D3D11Context::createRenderTarget() {
  ...
  DT_D3D11_THROW_FAILED(m_SwapChain->GetBuffer(
      0, __uuidof(ID3D11Texture2D),
      reinterpret_cast<void **>(backBuffer.GetAddressOf())));
  DT_D3D11_THROW_FAILED(m_Device->CreateRenderTargetView(
      backBuffer.Get(), nullptr, m_RenderTarget.GetAddressOf()));
}

void Destiny::D3D11Context::cleanUpRenderTarget() { m_RenderTarget->Release(); }

bool Destiny::D3D11Context::isVSync() { return m_VSync; }

void Destiny::D3D11Context::setVSync(bool enabled) { m_VSync = enabled; }

void Destiny::D3D11Context::resize(unsigned int width, unsigned int height) {
  if (m_Device != nullptr) {
    cleanUpRenderTarget();
    DT_D3D11_THROW_FAILED(m_SwapChain->ResizeBuffers(
...
void Destiny::D3D11Context::imGuiNewFrame() { ImGui_ImplDX11_NewFrame(); }

void Destiny::D3D11Context::destroyImGuiImpl() { ImGui_ImplDX11_Shutdown(); }
